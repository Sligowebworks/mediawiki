		...
		$text = preg_replace_callback(
			'!(?:                           # Start cases
			    ...
			    ISBN\s+(\b                  # ISBN, capture number as m[2]
                                     ...
                                     [0-9Xx]                 # check digit
                                   \b) |
			    BGZ:\s+([0-9]+) |		# BugID magic identifier as m[3]
			    SVN:\s+([0-9A-Za-z_/\-.]*\b) |   # SVN magic identifier as m[4]
			    WIKI:\s+([A-Za-z_\-:]+)	| # Wiki magic identifier as m[5]
			    TASK:\s+([0-9A-Za-z_\-:]+)	  # Bugzilla bug alias? as m[6]
	...
	function magicLinkCallback( $m ) {
		if ( substr( $m[0], 0, 1 ) == '<' ) {
			...
			if ( substr( $m[0], 0, 3 ) == 'RFC' ) {
				...
				$urlmsg = 'pubmedurl';
				$id = $m[1];
			} elseif ( substr( $m[0], 0, 4 ) == 'BGZ:' ) {
				$keyword = 'BGZ:';
				$urlmsg = 'bugurl';
				$id = $m[3];
			} elseif ( substr( $m[0], 0, 4 ) == 'SVN:' ) {
				$keyword = 'SVN:';
				$urlmsg = 'svnurl';
				$id = $m[4];
